...
public sealed class WeightedOperatorPolicy(Dictionary<GlyphType, float> weights) : IOperatorPolicy
{
    ...
    private readonly FrozenDictionary<GlyphType, float> _binaryOperators = OperatorsFilter.GetBinary(weights).ToFrozenDictionary();
...
{
    public static IDictionary<GlyphType, float> GetBinary(IDictionary<GlyphType, float> operators)
    {
